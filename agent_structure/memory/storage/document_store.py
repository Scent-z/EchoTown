...
class SQLiteDocumentStore(DocumentStore):
    ...
    # 对__new__方法不是很熟悉
    # 这里是为了确保同一个数据库路径只有一个实例 ，避免重复创建和资源浪费
    def __new__(cls, db_path: str = "./memory.db"):
        """单例模式，同一路径只创建一个实例"""
        abs_path = os.path.abspath(db_path)
        if abs_path not in cls._instances:
            instance = super(SQLiteDocumentStore, cls).__new__(cls)  # Python2写法, 等价于Python3的super().__new__(cls)
    ...
    def _init_database(self):
        ...
        # 创建记忆表
        # id    user_id    content    memory_type    timestamp    importance    properties    created_at    updated_at
        cursor.execute("""
        ...
        # 创建概念表
        # id    name    description    properties    created_at
        cursor.execute("""
        ...
        # 创建记忆-概念关联表
        # memory_id    concept_id    relevance_score    created_at
        cursor.execute("""
        ...
        # 创建概念关系表
        # from_concept_id    to_concept_id    relationship_type    strength    properties    created_at
        cursor.execute("""
        ...
        # 创建索引（B+Tree 索引）, 这样查数据时从全盘扫描O(n)变为O(log n)
        indexes = [
