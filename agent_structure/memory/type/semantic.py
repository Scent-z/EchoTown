...
class SemanticMemory(BaseMemory):
    ...
    # ✅️
    # 不够智能, 硬编码优先级, 没有根据文本语言智能选择, 若中文 + 英文都存在, self.nlp=中文模型
    def _init_nlp(self):
        """初始化NLP处理器 - 智能多语言支持"""
        try:
            import spacy  # 一个Python 自然语言处理（NLP）库, 分词或提取人名、地名等功能
            self.nlp_models = {}
    ...
    def _combine_and_rank_results(
        ...
        for memory_id, result in combined.items():
            ...
            # 有个问题, 由前面可知（图搜索结果并没有覆盖importance也没有做合并融合）这里的importance使用的是向量搜索得到的
            importance = result.get("importance", 0.5)
    ...
    # 提取出实体, 给Neo4j使用做准备；现在不适用于中英文混合的情况，中英文混合下不能较好地提取实体
    # ✅️
    ...
    def _store_linguistic_analysis(self, doc, text: str):
        ...
        try:
            ...
            # 建立词元之间的依存关系
            # 存储主语、谓语、宾语等的关系，如哪个词是哪个词的主语，哪个词是哪个词的定语
            for token in doc:
