...
class LLM:
    ...
    def _auto_detect_provider(self, api_key: Optional[str], base_url: Optional[str]) -> str:
        ...
        # 3. 根据API密钥格式判断（最低优先级，因为有些密钥可能很相似）
    ...
    # ✅️
    def chat_stream(self, messages: list[dict[str, str]], temperature: Optional[float] = None) -> Iterator[str]:
        """
        调用大语言模型进行思考，并返回流式响应。
        ...
        try:
            ...
            for chunk in response:
                ...
                if content:
                    print(content, end="", flush=True)  # 这个地方放在外部去打印更清晰，现在这种写法外部只能是一个空循环，外部再打印就会打两遍
                    yield content
            print()  # 在流式输出结束后换行
    ...
    def stream_invoke(self, messages: list[dict[str, str]], **kwargs) -> Iterator[str]:
        """
        流式调用LLM的别名方法，与chat_stream方法功能相同。
        保持向后兼容性。
        """
