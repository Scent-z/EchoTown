...
class MCPTool(Tool):
    ...
    # 为什么优先级低的反而在前面, 因为后面的处理会覆盖前面的值, 先设置低优先级的值（作为基础）再用高优先级的值覆盖它
    # ✅️
    ...
    def _discover_tools(self):
        """发现MCP服务器提供的所有工具"""
        try:
            from full_agent_structure.protocols.mcp.client import MCPClient
            import asyncio
            ...
            try:
                ...
                # 一个线程对应一个loop循环, loop不能在线程间共享, 这里是新开一个线程再新开一个loop(原因是这段代码可能处于外部一个异步loop循环中)
