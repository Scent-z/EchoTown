...
# 当添加一个 MCP 工具到 Agent 时，它会自动将 MCP 服务器提供的所有工具展开为独立的工具，让 Agent 可以像调用普通工具一样调用它们
...
class Tool(ABC):
    ...
    # 工具具备了自描述能力，通过get_parameters方法能够清晰地告诉调用者自己需要什么参数
    @abstractmethod
...
class AutoGeneratedTool(Tool):
    ...
    def _parse_parameters(self) -> List[ToolParameter]:
        ...
        # inspect.signature 看的是完整函数签名, get_type_hints 只关心类型注解
        sig = inspect.signature(self.method)  # 查看一个函数/方法的参数签名
        type_hints = get_type_hints(self.method)  # 用来读取函数、类、模块里的类型注解，会把参数和返回值的类型标注都拿出来
        docstring = inspect.getdoc(self.method) or ""  # 获取函数注释的
        ...
        for param_name, param in sig.parameters.items():
            ...
            # 获取类型
            param_type_hint = type_hints.get(param_name, str)  # str\int\float\bool\list\dict
            param_type = self._python_type_to_tool_type(param_type_hint)  # string\integer\number\boolean\array\object
