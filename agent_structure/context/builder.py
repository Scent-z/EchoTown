...
from ..tools.tools import MemoryTool
...
class ContextConfig:
    ...
    reserve_ratio: float = 0.15  # 生成余量（10-20%）, 为系统指令预留的比例
...
# 核心是 GSSC(Gather-Select-Structure-Compress)流水线，它将上下文构建过程分解为四个清晰的阶段。让我们深入了解每个阶段的实现细节
class ContextBuilder:
    ...
    def _gather(
        ...
        if self.memory_tool:
            try:
                # 搜索任务状态相关记忆
                state_results = self.memory_tool.execute(  # bug, 没有这个方法
                    "search",
    ...
    def _select(
        ...
        # 不懂因为后面有压缩操作, 为什么这里还要限制长度, 虽然_structure里会产生额外内容可能超过最大长度, 但这里这样做逻辑还是不太通
        # 先放入系统指令（不排序）
    ...
    # ✅️
    # 将散乱的信息包组织成清晰的分区, 优势有(1)可读性：清晰的分区让人类和模型都更容易理解上下文结构 (2)可调试性：问题定位更容易，可以快速识别哪个区域的信息有问题 (3)可扩展性：添加新的信息源只需要创建新的分区
    def _structure(
        self,
